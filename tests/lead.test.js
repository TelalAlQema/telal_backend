...
// modules must not load: mock.module rewires the import graph before app.js.
const created = [];
const fakePrisma = {
  submission: {
    create: async ({ data }) => {
...
mock.module("../src/lib/mailer.js", {
  namedExports: {
    sendCustomerConfirmation: async (data) => {
...
test("POST /api/v1/contact with a valid payload returns 200, persists a CONTACT lead and sends both emails", async () => {
  ...
  assert.equal(customer[0].data.source, "CONTACT");
  assert.equal(admin[0].data.name, "John Smith");
});

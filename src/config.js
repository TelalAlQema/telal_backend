...
const EnvSchema = z
  .object({
    ...
    // Optional so the application can boot without keys.
    // Verification should fail closed when the secret is absent.
    RECAPTCHA_SECRET: z
      .string()
      .trim()
      .optional(),
  ...
  .superRefine((env, ctx) => {
    // SMTP must be configured all-or-none.
    const smtpParts = [
      env.SMTP_HOST,
      env.SMTP_USER,
      env.SMTP_PASS,
...
export function loadEnv(path = ".env") {
  try {
    ...
    // Missing .env is fine in production/CI where environment
    // variables are injected directly.
    //
    // Any other .env failure must surface loudly.
    if (err?.code !== "ENOENT") {
      throw new EnvError(

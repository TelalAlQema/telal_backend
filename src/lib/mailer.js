...
function buildTransporter() {
  ...
  return nodemailer.createTransport({
    ...
    // secure:false + port 587 is the standard STARTTLS setup.
    secure: false,
...
function customerTemplate(data) {
  ...
  const messageHtml = data.message
    ? `
      <p>Your message:</p>
      <blockquote style="border-left:3px solid #4CAF50;margin:8px 0;padding:4px 12px;color:#444;">
        ${escapeHtmlLines(data.message)}
      </blockquote>
    `
    : "";
  ...
  return {
    ...
    html: `
      <div style="font-family:Segoe UI,Arial,sans-serif;background:#f6f9fc;padding:30px;border-radius:10px;border:1px solid #ddd;max-width:600px;margin:auto;">
        <h2 style="color:#4CAF50;margin:0 0 8px;">
    ...
    html: `
      <div style="font-family:Segoe UI,Arial,sans-serif;background:#fffefc;padding:30px;border:1px solid #ccc;border-radius:10px;max-width:600px;margin:auto;">
        ...
        <ul style="line-height:1.8;">
          ...
          <li>
            Services: ${escapeHtml(data.services)}
          </li>
...
/*
 * Both email functions catch and log failures.
 *
 * A failure to send an email must never break an HTTP
...
export async function sendCustomerConfirmation(data) {
  ...
  if (!transporter) {
    console.log(
      "[mailer] SMTP not configured — skipping customer confirmation email."
    );
...
/**
 * Send new lead notification to the admin.
 */
export async function sendAdminNotification(data) {
  ...
  if (!transporter) {
    console.log(
      "[mailer] SMTP not configured — skipping admin notification email."
    );
